...
const fetchQueries = async (userBranches: string[] = [], appNoFilter?: string): Promise<Query[]> => {
  try {
    ...
    const response = await fetch(`/api/queries?status=all&team=sales&includeBoth=true${branchParam}${appNoParam}`);
    ...
    // Filter queries marked specifically for sales team only
    ...
    queries.forEach(queryGroup => {
      queryGroup.queries.forEach((query, index) => {
        ...
        const isResolved = [
          ...
          'approved-by-sales', 'deferred-by-sales', 'otc-by-sales', 'waived-by-sales'
        ].includes(queryStatus);
        
        console.log(`🔍 Sales Query Status Check: ${queryGroup.appNo} - Query ${index + 1} - Status: ${queryStatus} - IsResolved: ${isResolved}`);
        ...
        if (!isResolved) {
          ...
          console.log(`✅ Sales Query EXCLUDED (resolved): ${queryGroup.appNo} - Status: ${queryStatus}`);
        }
      });
    });
    
    console.log(`📊 Sales Dashboard: Total individual queries after filtering: ${individual.length} (pending only)`);
    return individual;
  }, [queries]);
