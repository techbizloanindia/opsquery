...
export default function SalesQueriesResolved({ searchAppNo }: SalesQueriesResolvedProps = {}) {
  ...
  const fetchResolvedQueries = async () => {
    try {
      ...
      // Fetch all queries, then filter for resolved ones by Sales team
      const response = await fetch(`/api/queries?team=sales&status=all${branchParam}`);
      ...
      if (result.success) {
        ...
        result.data.forEach((query: any) => {
          // Check if this query was resolved by Sales team or is marked for Sales
          const isForSalesTeam = query.markedForTeam === 'sales' || 
                                 query.markedForTeam === 'both' ||
                                 query.team === 'sales' ||
                                 query.resolvedByTeam === 'Sales';
            ...
            query.queries.forEach((subQuery: any, index: number) => {
              if (resolvedStatuses.includes(subQuery.status)) {
                ...
                expandedResolvedQueries.push({
                  ...
                  resolvedBy: subQuery.resolvedBy || query.resolvedBy || 'Sales Team',
